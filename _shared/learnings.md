...
## [2026-09-07] [req-net-func-01]
**교훈**: 요구사항→task 변환에서 `진행현황`의 기계적 매핑(요구사항_TASK_전환_Format §진행현황: 기획중→분석·요약→설계문서)은 **기본값일 뿐**이다. 사용자가 명시적으로 "진행/구현"을 지시하면 `성격`을 코드 구현으로 상향하고 그 상향을 log.md의 [DECISION]으로 남긴다(원문 시트는 여전히 기획중이므로 변환 시점의 임의 결정임을 표시). 또한 요구사항 [Worker Settings]가 워커를 지정해도, 하네스가 서브에이전트 spawn을 억제하고 사용자가 자율 진행을 승인한 맥락에서는 **Orchestrator가 도메인 서브에이전트 규약을 흡수해 inline 실행**하는 것이 정당하다 — 별도 모델 호출(승인·쿼터·cold-start)을 아낀다.
**부수 교훈 — 검증은 명령된 빌드도구가 없어도 성립한다**: 요구사항이 Gradle을 요구하고 이 환경엔 gradle·redis-cli가 없었으나 Java 21은 있었다. 빌드 설정(build.gradle.kts)은 산출물로 제공하되 [검증 기준]은 `javac`+**순수 JDK 소켓 하니스**로 판정(PASS 7/7)했다. 명령된 도구의 부재가 검증 불가를 뜻하지 않는다 — 판정에 필요한 최소 실행경로(JDK만)로 대체하고 그 사실을 log에 남긴다.
**worker**: orchestrator 단독(요청문 변환·구현·JDK 검증). 워커 호출 없음 — inline 실행([임의 결정 2]).

## [2026-09-07] [req-str-func-01]
...
**부수 교훈 — redis-cli 상호운용은 CI 스텝으로만 검증 가능(로컬 미설치)**: 실행 바이너리라 Gradle testImplementation처럼 JUnit화가 안 된다. ubuntu 러너에 `apt-get install redis-tools` → `installDist` 런처를 백그라운드 기동 → 공식 redis-cli로 왕복 assert. 로컬에서 못 돌리는 스텝의 리스크는 (a) **서버 기동 경로를 로컬에서 소켓 PING으로 선검증**(installDist 런처가 포트 인자로 바인딩·PONG)하고 (b) redis-cli 기대 출력은 **non-TTY 기본 raw 모드**(정수 `1`·벌크 `bar`·상태 `OK`, `(integer)`/따옴표 없음, 배열 개행분리) 기준으로 작성해 줄였다. GitHub Actions 기본 셸이 `bash -eo pipefail`이므로 실패 처리(`|| true`, `if` 조건 내 substitution)는 그에 맞춰 방어.
