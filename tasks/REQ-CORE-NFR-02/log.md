...
[2026-09-15 00:41] [VERIFICATION] 검증기준② redis-cli 바이너리 상호운용까지 충족(② 완전 완료). CI에 "redis-cli interop smoke" 스텝 추가(.github/workflows/ci.yml): ubuntu redis-tools 설치 → installDist 런처로 6390 기동 → 공식 redis-cli 로 PING/SET·GET/INCR/RPUSH·LRANGE/HSET·HGET/TYPE 왕복 + WRONGTYPE 에러 표면화 assert. 로컬(redis-cli 미설치)은 런처 기동 경로만 소켓 PING(+PONG)으로 선검증, redis-cli 자체는 Linux 러너에서 실행. 결과: run 6a3a668 = success, step 5 "redis-cli interop smoke"=success(build&test 74 그린도 동반). → NFR-02 검증기준 ①②③④ 전부 충족.
